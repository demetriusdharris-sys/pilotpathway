...
**Known open bugs:**
- **No quiz card is approved, so no student sees a quiz and `objective_mastery` — the only reportable stream — is still empty.** The machinery is built and verified; the blocker is a CFI reviewing the 108 drafted cards in `docs/cards/` (twelve of the sixteen Stage 1 lessons). Sent to a CFI Sep 22 2026; no answer yet. That is a person, not engineering, and it is the single biggest thing between this product and outcome data for a school.
