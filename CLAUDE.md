...
**Known open bugs:**
- **No quiz card is approved, so no student sees a quiz and `objective_mastery` — the only reportable stream — is still empty.** The machinery is built and verified, and since Sep 23 2026 a CFI can do the reviewing in the app at `/review/cards` (see below). The blocker is now purely a CFI's time on the 144 drafted cards in `docs/cards/`. **All 144 rows read `draft` as of Sep 24 2026, confirmed against the database** — an earlier note here claimed one had been approved during testing, which was wrong: only the refusal paths were exercised, and refusals write nothing. **Stage 1 is now fully covered — all sixteen lessons, all 48 objectives, three cards each** (Sep 22 2026). 22 cards carry a `[CFI: confirm value]` gap and 17 carry an open question for the reviewer; **a card with a gap must not be approved as it stands**, because the placeholder would render to a student. Sent to a CFI Sep 22 2026; no answer yet. That is a person, not engineering, and it is the single biggest thing between this product and outcome data for a school.
...
- **The reviewer's name is carried in the `?as=` URL parameter, not stored.** A reviewer may be a guest CFI on a borrowed account; inventing a profile column would be a schema change to solve a form problem. The name is written against the row on approval, which is what makes an approval a person rather than a click.

**Still open:** the value-gap refusal is untested end to end; a non-reviewer account has not been confirmed to see "Nothing here for this account"; and an approved question has not been watched appearing in a practice test, which is the real proof that review connects to serving. The five weather questions are test content — `update public.question_bank set review_status = 'draft', reviewed_by = null, reviewed_at = null, review_note = null;` puts them all back, and any `[CFI: confirm value]` text added by hand for testing must be stripped from the stem.
...
**Partly verified on the live site Sep 23 2026** as `admin`, against all 144 draft cards. **What was watched:** a flagged card refused approval with an empty note; a card with a value gap had Approve greyed out; and a card with **both** showed the doubt panel while still refusing approval, which is the case that proves the two markers are not treated alike. **What was not:** no card has been approved, so the write path — the reviewer's name and their answer landing on the row — is unexercised, and confirmed unexercised against the database Sep 24 2026.
