...
- **A commit message is not a record of apply state.** Migration commits say "(not yet applied)" because that was true when the file was written; they are never amended once the migration is applied. **As of Sep 17 2026, every migration `0001` through `0020` is applied to production and verified.** Do not infer from a commit message that a migration is pending — ask, or check the database.
...
**Known open bugs:**
- **No quiz card is approved, so no student sees a quiz and `objective_mastery` — the only reportable stream — is still empty.** The machinery is built and verified; the blocker is a CFI reviewing the 54 drafted cards in `docs/cards/` (six lessons: stalls, IMSAFE and PAVE, pilot in command, the traffic pattern, preflight, weather). Sent to a CFI Sep 22 2026; no answer yet. That is a person, not engineering, and it is the single biggest thing between this product and outcome data for a school.
...
**2. Progress tracking was a checkbox — paid Sep 17 2026 for the student's own view; still owed for staff.** `lesson_progress` is still one three-state flag per lesson, but it is no longer all a student sees: each objective on a lesson page now reads Shown / Keep going / Not shown yet, and each stage on the dashboard reads "N of M objectives shown" (see "What a student has shown" below). What is still owed: nobody but the student can see any of it. A teacher dashboard, and the aggregate reporting a school renews a contract for, are a separate project on top of the same view — and both need the consent rules from `0006` applied, not bypassed.
...
- **Recording a CFI's answers is `docs/cards/recording-approvals.md`** — copy-paste SQL for approving, cutting, and withdrawing a card, written for the founder. A wording change is never made in the database: the markdown is the source, and the sync would overwrite it.
