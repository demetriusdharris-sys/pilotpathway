...
**An explanation must never name a choice by its letter, and the importer refuses one that does.** Choice order is shuffled per attempt and the permutation is stored per answer, so the letter a student sees is not the letter the author wrote — "B is wrong because…" points at the wrong option in the worst place, the explanation shown after grading to a student trying to understand their mistake. **All 24 of the first questions broke this**, including the five written to prove the pipeline worked, which is why it is a check rather than a rule. The check carries a lookbehind for `Class`, `Category`, `Part` and similar, because "Class B is the one place you need a clearance" is correct prose and the airspace and regulations batches are full of it — a check that rejects good work is a check somebody deletes. Two sibling rules live in `docs/questions/AUTHORING-RULES.md` and are not enforced in code: vary which letter is correct (the first draft had 22 of 24 on `A`, which teaches a reviewer to stop reading distractors), and do not ask the same fact twice in one area (eight weather questions are served per full test, so a duplicate can land twice on one paper).

**A batch is one knowledge area sized to three times its blueprint slots**, not twenty per ACS code as the rules first said. The reason the rule existed — a reviewer who works one area at a time finishes — is served either way, and area-sized batches distribute by what the test actually weights. Weather is done: 24 questions, 18 ACS codes, no value gaps, so a CFI can approve the lot without sending numbers back first.
